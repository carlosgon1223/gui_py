...
class Gui:
    ...
    def _manage_message(self, msg_type: _WsType, message: dict):
        try:
        client_id = self._get_client_id_from_message(msg_type, message)
        self.__set_client_id_in_context(client_id)
        g.ws_client_id = client_id
    ...
    def _get_client_id_from_message(self, msg_type: _WsType, message: dict) :
        ...
        return message.get(Gui.__ARG_CLIENT_ID)

    def _handle_message(self, msg_type: _WsType, message: dict):
    ...
    def _handle_exception(self, e: Exception, message: dict):
        if isinstance(e, AttributeError) and (name := message.get("name")):
            try:
                names = self._get_real_var_name(name)
                var_name = names[0]                         
                self.__front_end_update(var_name, None, True, None, None)
            except Exception as e:
                self.__logger.error(f"Error handling exception: {e}")
        else:
            self.__logger.error(f"Error managing message: {e}")
